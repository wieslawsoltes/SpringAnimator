...
public class SpringAnimator : Animator<double>
{
    ...
    public override double Interpolate(double progress, double oldValue, double newValue)
    {
        ...
        //Debug.WriteLine($"Interpolate {progress} -> {t} -> {t * newValue} ({oldValue} -> {newValue})");
        return ((newValue - oldValue) * t) + oldValue;
    }
}
...
/// </summary>
public class TransformAnimator1 : Animator<double>
{
    SpringAnimator? _doubleAnimator;
    ...
    public override IDisposable? Apply(Animation animation, Animatable control, IClock? clock, IObservable<bool> obsMatch, Action? onComplete)
    {
        ...
        if (typeof(Transform).IsAssignableFrom(Property.OwnerType))
        {
            ...
            {
                _doubleAnimator = new SpringAnimator();
